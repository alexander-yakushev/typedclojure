           ...
           (doall
             (for [[n# [fields# & {:as popts#}]] ts#]
               (let [vs# (seq
                     ...
                     methods# (let [opts# (free-ops/with-bounded-frees2 opts# names# bnds#)]
                                (into {}
...
(defn jsnominal-entry [[n [binder & {:as jopts}]] opts]
  (let [names (when (seq binder)
        ...
        fields (let [opts (free-ops/with-bounded-frees2 opts names bnds)]
                  (into {}
                        (for [[mname mtype] (:fields jopts)]
                          [mname (c/abstract-many names (prs/parse-type mtype opts) opts)])))
        ctor (when-let [ctor (:ctor jopts)]
                (let [opts (free-ops/with-bounded-frees2 opts names bnds)]
                  (c/abstract-many names (prs/parse-type ctor opts) opts)))
        ancestors (let [opts (free-ops/with-bounded-frees2 opts names bnds)]
                    (into #{}
