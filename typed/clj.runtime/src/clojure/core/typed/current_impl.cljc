...
#?(:cljs :ignore :default
(def ^:private with-bounded-frees2 #((requiring-resolve 'typed.cljc.checker.free-ops/with-bounded-frees2) %1 %2 %3)))
#?(:cljs :ignore :default
  ...
  (let [opts (assoc opts :typed.clj.checker.parse-unparse/parse-type-in-ns current-ns)
        ...
        ms (into {} (map (fn [[knq v*]]
                           (let [_ (when (namespace knq)
                                 ...
                                 (delay-type
                                   (let [opts (-> opts
                                                  (with-bounded-frees2 (mapv :name (force-type fs opts)) (force-type bnds opts))
        ...
        pos-ctor (let [bfn (fn []
                             (if args
                               (let [args (force-type args opts)
                                     ...
                                     opts (with-bounded-frees2 opts args bnds)]
                                 (Poly* args bnds
                                        (make-FnIntersection
