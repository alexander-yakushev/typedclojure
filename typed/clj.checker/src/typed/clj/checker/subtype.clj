    ...
    (if (AND (= :Poly (:kind s))
      ...
      (if (AND (= (:nbound s) (:nbound t))
        ...
        (let [;instantiate both sides with the same fresh variables
          ...
          (if (subtypeA* A b1 b2 (-> opts (free-ops/with-bounded-frees2 names bbnds1)))
            A
            (report-not-subtypes s t)))
        unknown-result)
      unknown-result))
    ...
    (if (r/-Poly? t)
      (when (AND (= :PolyDots (.kind s))
        ...
        (let [;instantiate both sides with the same fresh variables
          ...
          (when (= bbnds1 bbnds2)
            (subtypeA* A b1 b2 (free-ops/with-bounded-frees2 opts names bbnds1)))))
      (if (OR ;use unification to see if we can use the Poly type here
              (case (.kind s) 
                :Poly (let [names (c/Poly-fresh-symbols* s)
                            ...
                            opts (free-ops/with-bounded-frees2 opts names bnds)
                            b1 (c/Poly-body* names s opts)
      ...
      (cond
        ...
        (and (r/Poly? t)
             ...
             (let [names (c/Poly-fresh-symbols* t)
               ...
               (subtypeA* A s b
                          (-> opts (free-ops/with-bounded-frees2 names bbnds)))))
        A
  ...
  (let [;instantiate both type functions with the same names
    ...
    (if (and (= (:nbound S) (:nbound T))
             ...
             (every?' (fn [lbnd rbnd]
                      ...
                      sbnds tbnds))
      (subtypeA* A sbody tbody (free-ops/with-bounded-frees2 opts names
                                 ;;FIXME which side?
                                 sbnds))
      (report-not-subtypes S T))))
